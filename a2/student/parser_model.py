...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_weight= nn.init.xavier_uniform_(torch.empty(self.embed_size*self.n_features, self.hidden_size))
        self.embed_to_hidden_bias= nn.init.uniform_(torch.empty(1, self.hidden_size))
    ...
    def embedding_lookup(self, w):
        ...
        # 使用 torch.index_select 来从 self.embeddings 中选择词嵌入
        # 参数 '0' 表示我们要在第一个维度 (也就是行) 上选择
        # 这个操作将返回一个新的张量，包含 w_reshaped 中索引指定的词嵌入
        x_reshaped = torch.index_select(self.embeddings, 0, w_reshaped)

        # 最后，我们把选出的词嵌入 reshape 回原有的尺寸
        # 这样每个原始输入的样本将对应到一个由词嵌入组成的二维张量
        x = x_reshaped.view(w.shape[0], -1)
