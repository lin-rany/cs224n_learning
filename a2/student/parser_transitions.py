...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        ...
        need_pop_idx=[]
        for idx,parse in enumerate(batch):
            parse.parse_step(transitions[idx])
        unfinished_parses=[parse for parse in unfinished_parses if not((not parse.buffer) and len(parse.stack)==1)]
