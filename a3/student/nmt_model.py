...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        enc_hiddens, (last_hidden, last_cell)=self.encoder(X) # (src_len, b, h*2) ,(2, b, h),
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in YTS: #Y_t (1,b,e)
            Y_t=torch.squeeze(Y_t) #(b,e)
            Ybar_t=torch.cat((Y_t,o_prev),dim=1)
            dec_state,combined_output,e_t=self.step(Ybar_t=Ybar_t,dec_state=dec_state,enc_hiddens=enc_hiddens,
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        b_dec_hidden=torch.unsqueeze(dec_hidden,dim=-1)
        e_t=torch.bmm(enc_hiddens_proj,b_dec_hidden)
        ...
        b_alpha_t=torch.unsqueeze(alpha_t,dim=1)
        a_t=torch.bmm(b_alpha_t,enc_hiddens)
